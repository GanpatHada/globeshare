...
const FeedActions = ({postId,post}) => {
  ...
  const handleLike=()=>{
    ...
    if(isLiked(user.uid,postId))
      return handleLikesInServer(postId,'UNLIKE');
  ...
  return (
    <section className="feed-actions">
      <ul>
        <li>
          <button title={isLiked(user.uid,postId)?"UnLike":'Like'} onClick={handleLike} className="like-btn">
            {isLiked(user.uid,postId)?<AiFillHeart id="fill-heart-icon" />:<AiOutlineHeart id="outlined-heart" />} 
          </button>  
        </li>
        <li>
      ...
      <ul>
        <li>
          <button title="Bookmark" onClick={()=>handleBookmarkInServer(postId)}>
